...
import { YouDefaultError } from "../models/errors/youdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | YouDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | YouDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new YouDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new YouDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
